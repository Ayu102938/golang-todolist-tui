...
## Phase 4: 仕上げ（優先度：低） ✅ 2026-07-03

- [x] **4-1: 設定ファイル対応** — `config.json`（`~/.config/my-tui-app/config.json`）から `defaultCategory` と `theme` を読み込み。
- [x] **4-2: カラーテーマ** — default / dark / light の3種プリセット。`config.json` の `"theme"` で切り替え。
- [x] **4-3: i18n 基盤** — `i18n.go` に `messages` 構造体 + `japaneseMessages`。全 UI 文字列を `msg.*` 経由で参照。

**結果**: ビルド + 29 テスト合格。`config.json` でカテゴリ名・配色を設定可能。全 UI 文字列を外部化し多言語対応の土台を整備。新規ファイル: `config.go`, `i18n.go`。
